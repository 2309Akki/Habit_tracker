// Simple User Store - Perfect for Your Schema
    ...
    try {
      const user = await prisma.user.create({
        data: {
          ...
          password: userData.password, // Store pre-hashed password
...
export const userData = {
  ...
  async addDate(userId: string, habitName: string, date: string) {
    try {
      ...
      let habitGroup = habits.find(h => h[habitName]);
  ...
  async removeDate(userId: string, habitName: string, date: string) {
    try {
      ...
      const habits = [...user.habits];
      const habitGroup = habits.find(h => h[habitName]);
      ...
      // Remove date
      const dates = habitGroup[habitName].filter(d => d !== date);
